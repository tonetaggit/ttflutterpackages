...
//     UIColor *markerColor = [UIColor colorWithHue:hue / 360.0
//                                       saturation:1.0
//                                       brightness:0.7
//                                            alpha:1.0];

//     // Define a fallback marker size (Google Maps default marker size)
//     CGSize fallbackSize = CGSizeMake(40, 65);

//     @try {
//         if ([NSThread isMainThread]) {
//             image = [GMSMarker markerImageWithColor:markerColor];
//         } else {
...
//         // 🧩 If Google Maps fails to return a valid image, build a simple fallback one
//         if (!image || image.size.width == 0 || image.size.height == 0) {
//             UIGraphicsImageRenderer *renderer =
//                 [[UIGraphicsImageRenderer alloc] initWithSize:fallbackSize];
//             image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull context) {
//                 [markerColor setFill];
//                 UIBezierPath *circle = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, 40, 40)];
//                 [circle fill];
//             }];
//         }

//     } @catch (NSException *exception) {
//         NSLog(@"⚠️ GMSMarker markerImageWithColor crashed: %@", exception);
//         // fallback marker using UIGraphicsImageRenderer (safe modern API)
//         UIGraphicsImageRenderer *renderer =
//             [[UIGraphicsImageRenderer alloc] initWithSize:fallbackSize];
//         image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull context) {
//             [markerColor setFill];
