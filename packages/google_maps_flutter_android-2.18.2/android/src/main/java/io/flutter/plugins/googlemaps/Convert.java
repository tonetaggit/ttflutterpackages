...
class Convert {
  ...
  // Native-level icon cache for marker bitmaps
  // Caches decoded BitmapDescriptor objects to prevent redundant decoding
  // Key format: "assetKey_widthxheight_density"
  // Capacity: 150 entries (supports ~60 icon types with 2-3 size variants)
  private static final LruCache<String, BitmapDescriptor> bitmapCache =
      new LruCache<String, BitmapDescriptor>(150) {
        @Override
        protected void entryRemoved(boolean evicted, String key,
                                   BitmapDescriptor oldValue, BitmapDescriptor newValue) {
  ...
  /**
   * Generates a unique cache key for bitmap descriptors based on asset path and dimensions.
   *
   * @param assetKey The asset path (e.g., "flutter_assets/assets/charge/marker_1.svg")
   * @param width Target width in pixels
   * @param height Target height in pixels
   * @param density Screen density
   * @return Unique cache key string
   */
  private static String generateBitmapCacheKey(
      String assetKey, int width, int height, float density) {
    return assetKey + "_" + width + "x" + height + "_" + density;
  }

  /**
   * Clears the bitmap cache. Useful for memory management or when changing themes.
   * Call this when app goes to background or memory is low.
   */
    ...
    switch (scalingMode) {
      case AUTO:
        ...
        try (InputStream inputStream = assetManager.open(assetKey)) {
          // Configure BitmapFactory options for optimal performance
          BitmapFactory.Options options = new BitmapFactory.Options();
          // Use ARGB_8888 for standard format support across all devices
          // This prevents GPU format compatibility issues (RGBA_FP16) on low-end devices
          options.inPreferredConfig = Bitmap.Config.ARGB_8888;
          // Disable dithering for faster decoding
          // options.inDither = false;
          // Disable scaling during decode
          options.inScaled = false;

          Bitmap bitmap = BitmapFactory.decodeStream(inputStream, null, options);
          
          if (bitmap == null) {
            throw new IllegalArgumentException("Failed to decode bitmap from asset: " + assetName);
          }
          ...
          try {
            if (width != null || height != null) {
              ...
              // Generate cache key with final dimensions
              String cacheKey = generateBitmapCacheKey(assetKey, targetWidth, targetHeight, density);

              // Check cache first to avoid redundant bitmap processing
              BitmapDescriptor cachedDescriptor;
              synchronized (bitmapCache) {
                cachedDescriptor = bitmapCache.get(cacheKey);
              }
              
              if (cachedDescriptor != null) {
                // Cache hit - return immediately without scaling
                return cachedDescriptor;
              }

              // Cache miss - scale bitmap and create descriptor
              Bitmap scaledBitmap = toScaledBitmap(bitmap, targetWidth, targetHeight);
              
              // Recycle original bitmap if it's different from scaled bitmap
              ...
              Double imagePixelRatio = assetMap.getImagePixelRatio();
              if (imagePixelRatio == null) {
                throw new IllegalArgumentException("imagePixelRatio cannot be null for asset: " + assetName);
              }
              
              final float scale = density / imagePixelRatio.floatValue();
              ...
              // Cache miss - scale and create descriptor
              Bitmap scaledBitmap = toScaledBitmap(bitmap, scale);
            ...
            // Make sure to recycle bitmap if any error occurs during processing
            if (bitmap != null) {
              bitmap.recycle();
            }
            throw e;
          ...
          throw new IllegalArgumentException("Cannot open asset: " + assetName, e);
        } catch (Exception e) {
          throw new IllegalArgumentException("Error processing asset: " + assetName, e);
        }
      case NONE:
        // No scaling required, use asset directly
        return bitmapDescriptorFactory.fromAsset(assetKey);
      default:
        // Handle any future scaling modes
        throw new IllegalArgumentException("Unsupported bitmap scaling mode: " + scalingMode);
    }
  }
