...
class PolygonsController {
  ...
  void addPolygons(@NonNull List<Messages.PlatformPolygon> polygonsToAdd) {
    ...
    if (polygonsToAdd.size() > threshold) {
      addPolygonsInBatches(polygonsToAdd, 0, batchSize);
    } else {
  ...
  private void addPolygonsInBatches(List<Messages.PlatformPolygon> polygons, int startIndex, int batchSize) {
    int endIndex = Math.min(startIndex + batchSize, polygons.size());
    
    for (int i = startIndex; i < endIndex; i++) {
      addPolygon(polygons.get(i));
    }
    
    if (endIndex < polygons.size()) {
      mainHandler.post(() -> addPolygonsInBatches(polygons, endIndex, batchSize));
    }
  }
