...
class GoogleMapController {
  ...
  // Android-specific marker cache methods

  /// Android-only: Method channel for marker cache operations
  MethodChannel? get _markerCacheChannel {
    if (!_isAndroid) return null;
    return MethodChannel(
        'plugins.flutter.dev/google_maps_android_$mapId/marker_cache');
  }
  ...
  /// Android-only: Initialize native marker cache with all marker data.
  /// This should be called once with the complete marker dataset.
  /// Returns true on success, false if not on Android or if initialization failed.
  ///
  /// Usage:
  /// ```dart
  /// final markerData = markers.map((m) => {
  ///   'merchantId': m.markerId.value,
  ///   'latitude': m.position.latitude,
  ///   'longitude': m.position.longitude,
  ///   'chargerType': 'fast', // or 'slow'
  ///   'status': 'available', // 'available', 'charging', 'unavailable'
  ///   'partyId': 'service_provider_id',
  ///   'isTataEverified': true,
  ///   'isEverified': false,
  ///   'paymentEnabled': true,
  ///   'rfidEnabled': false,
  ///   'connectorPowers': [50.0, 120.0],
  ///   'pin': 'marker_icon_url',
  ///   'selectedPin': 'selected_marker_icon_url',
  /// }).toList();
  /// await controller.initializeMarkerCache(markerData);
  /// ```
  Future<bool> initializeMarkerCache(
      List<Map<String, dynamic>> markerData) async {
    final channel = _markerCacheChannel;
    if (channel == null) return false;

    try {
      await channel.invokeMethod('initializeCache', markerData);
      return true;
    } catch (e) {
      print('Error initializing marker cache: $e');
      return false;
    }
  }

  /// Android-only: Apply filter to cached markers.
  /// Returns true on success, false if not on Android or if filter failed.
  ///
  /// Usage:
  /// ```dart
  /// final filter = {
  ///   'serviceProviders': ['provider1', 'provider2'],
  ///   'chargerTypes': ['fast'],
  ///   'statuses': ['available', 'charging'],
  ///   'requireTataVerified': true,
  ///   'requireEverified': false,
  ///   'requirePaymentEnabled': true,
  ///   'requireRfidEnabled': false,
  ///   'minPower': 50.0,
  ///   'maxPower': 150.0,
  /// };
  /// await controller.applyMarkerFilter(filter);
  /// ```
  Future<bool> applyMarkerFilter(Map<String, dynamic>? filterCriteria) async {
    final channel = _markerCacheChannel;
    if (channel == null) return false;

    try {
      await channel.invokeMethod('applyFilter', filterCriteria);
      return true;
    } catch (e) {
      print('Error applying marker filter: $e');
      return false;
    }
  }

  /// Android-only: Clear all filters and show all cached markers.
  /// Returns true on success, false if not on Android or if clear failed.
  Future<bool> clearMarkerFilter() async {
    final channel = _markerCacheChannel;
    if (channel == null) return false;

    try {
      await channel.invokeMethod('clearFilter');
      return true;
    } catch (e) {
      print('Error clearing marker filter: $e');
      return false;
    }
  }

  /// Android-only: Check if marker cache is initialized.
  /// Returns true if cache contains markers, false otherwise.
  Future<bool> isMarkerCacheInitialized() async {
    final channel = _markerCacheChannel;
    if (channel == null) return false;

    try {
      final result = await channel.invokeMethod<bool>('isInitialized');
      return result ?? false;
    } catch (e) {
      print('Error checking marker cache initialization: $e');
      return false;
    }
  }

  /// Android-only: Get the number of markers in the cache.
  /// Returns the cache size, or 0 if not on Android or if query failed.
  Future<int> getMarkerCacheSize() async {
    final channel = _markerCacheChannel;
    if (channel == null) return 0;

    try {
      final result = await channel.invokeMethod<int>('getCacheSize');
      return result ?? 0;
    } catch (e) {
      print('Error getting marker cache size: $e');
      return 0;
    }
  }
